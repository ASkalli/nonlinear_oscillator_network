...
class CMAESOpt:
    """
    PyTorch version of  CMA_opt (full covariance CMA-ES),
    same structure, variable names, and ask/tell behavior.
    """
    ...
    def __init__(self, N_dim, pop_size, select_pop, sigma_init, mean_init,
        ...
        # ---- mean_init checks / reshape (match numpy behavior) ----
        ...
        # ---- Strategy parameter setting: Adaptation (same formulas) ----
        N = self.N_dim
        ...
        self.population = None  # (N_dim, pop_size)
        self.counteval = 0
    ...
    def ask(self) -> torch.Tensor:
        """
        Generate a new population, same as numpy:
        population = xmean + sigma * (B @ (D * randn(N, pop)))
        Returns: (pop_size, N_dim) 
        """
        Z = torch.randn((self.N_dim, self.pop_size), device=self.device, dtype=self.dtype)
        BDZ = self.B @ (self.D * Z)  # (N, pop)

        self.population = self.xmean + (self.sigma * BDZ)
    ...
    def tell(self, reward_table) -> None:
        """
        Update CMA-ES parameters from rewards.
        Assumes minimization.
        """
        # reward_table checks / to tensor
        ...
        # ---- Update mean ----
        # population: (N, pop). Select best columns -> (N, select_pop)
        best_pop = self.population[:, arindex[:self.select_pop]]
        # Weighted mean: (N, select_pop) @ (select_pop,1) -> (N,1)
        self.xmean = best_pop @ self.weights.reshape(self.select_pop, 1)

        # ---- Update evolution paths ----
        y = (self.xmean - xold) / (self.sigma + self.epsilon)  # (N,1)
        ...
        # artmp = (1/sigma) * (best_pop - xold repeated)
        artmp = (best_pop - xold.repeat(1, self.select_pop)) / (self.sigma + self.epsilon)  # (N, select_pop)
        ...
        # rank-mu update: artmp @ diag(weights) @ artmp.T
        ...
        # ---- Update eigen decomposition less frequently ----
        if (self.counteval % self.eigen_update_frequency) == 0:
            ...
            # clamp evals for stability before sqrt
        ...
        current_best_solution = self.population[:, current_best_index]  # (N,)
...
# -------------------------------
# Params (ADJUST N_dim manually will code the loop later )
# -------------------------------
N_dim   = 16750       # <-- full covariance: start small
dtype   = torch.float32
device  = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
n_loops = 5
warmup_loops = 5

# pop / selection (typical: select ~ pop/2)
...
CMA_optimizer = CMAESOpt(
    N_dim=N_dim,
    pop_size=pop_size,
    select_pop=select_pop,
    sigma_init=1e-1,
    mean_init=init_pos,   # (N,1) or (N,)
    device=device,
    dtype=dtype,
)
...
print(f"N_dim={N_dim}, pop_size={pop_size}, select_pop={select_pop}")
...
# internal state 
for attr in ["xmean", "pc", "ps", "B", "D", "C", "invsqrtC", "population"]:
