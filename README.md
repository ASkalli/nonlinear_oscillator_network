...
$$
\dot{\mathbf{h}}^{[l]}(t) = -\alpha \mathbf{h}^{[l]}(t) + \boldsymbol{\beta} \cdot \sin\left(W_{\text{rec}}^{[l]} \mathbf{h}^{[l]}(t) + \gamma W_{\text{in}}^{[l]} \mathbf{h}^{[l-1]}(t) + \mathbf{b}^{[l]}\right) + \mu \mathcal{N}(0, 1) , \text{for}~l>1
$$
